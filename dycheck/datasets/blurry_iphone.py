...
@gin.configurable()
class BlurryiPhoneParser(NerfiesParser):
    ...
    def load_rgba(self, time_id: int, camera_id: int) -> np.ndarray:
        return super().load_rgba(time_id, camera_id, use_undistort=False, use_blurry=True)
    ...
    def load_depth(
        ...
        depth_path = osp.join(
            self.data_dir, "depth/2x", frame_name + ".npy"
        )
        depth = io.load(depth_path) * self.scale
        camera = self.load_camera(time_id, camera_id)
        # The original depth data is projective; convert it to ray traveling distance.
    ...
    def load_normal(
        ...
        frame_name = self._frame_names_map[time_id, camera_id]
        normal_path = osp.join(
            self.data_dir, "normal", f"{self._factor}x", frame_name + ".npy"
        )
        normal = io.load(normal_path)
        return normal # this is local normal

    def load_camera(
        self, time_id: int, camera_id: int, use_perturb: Optional[bool] = None, **_
    ) -> geometry.Camera:
...
@gin.configurable()
class BlurryiPhoneDatasetFromAllFrames(NerfiesDatasetFromAllFrames):

    __parser_cls__: Parser = BlurryiPhoneParser
