...
class NerfiesParser(Parser):
    ...
    def load_rgba(
        ...
        *,
        ...
        use_blurry: Optional[bool] = False,
    ) -> np.ndarray:
        ...
        rgb_folder = "rgb"
        if use_undistort:
            rgb_folder = "rgb_undistort"
        elif use_blurry:
            rgb_folder = "blurry_rgb"
    ...
    def load_camera(
        ...
        *,
        use_undistort: Optional[bool] = None,
        use_distort: Optional[bool] = None,
        **_,
    ) -> geometry.Camera:
        if use_undistort is None:
            use_undistort = self.use_undistort
        if use_distort is None:
            use_distort = True
        
        if not use_distort:
            camera_folder = "camera"
        else:
            camera_folder = "camera_interp"

        frame_name = self._frame_names_map[time_id, camera_id]
        camera = (
            geometry.Camera.fromjson(
                osp.join(self.data_dir, camera_folder, frame_name + ".json")
            )
...
class NerfiesDataset(Dataset, MetadataInfoMixin):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)        # check the total training frames
        (
            _frame_names,
            _,
            _,
        ) = self.parser.load_split('train')
        self._num_frames = len(_frame_names)
    ...
    def __init__(self, **kwargs):
        ...
        if self.training:
            ...
            self.rays = struct.Rays(
                ...
                pixels = np.concatenate([r.pixels for r in rays], axis=0),
                metadata=struct.Metadata(
                    time=np.concatenate(
