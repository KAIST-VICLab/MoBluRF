...
def get_arc_traj(
    ...
    **_,
    ...
    lookat = ref_camera.optical_axis + ref_camera.position
    up = ref_camera.up_axis
    positions = [
        matv(
        ...
        + lookat
        for d in np.linspace(-degree / 6, degree / 8, num_frames)
    ]
    cameras = [ref_camera.lookat(p, lookat, up) for i, p in enumerate(positions) if i>=11 and i <=40]
    cameras = cameras + [cameras[i] for i in range(len(cameras)-1, -1, -1)]
    return cameras
...
def get_lemniscate_traj(
    ...
    **_,
    ...
    a = np.linalg.norm(ref_camera.position - lookat) * np.tan(degree / 360 * np.pi )
