...
def procrustes_analysis(X0, X1):  # [N,3] X0 is target X1 is src
    ...
    U, S, V = jnp.linalg.svd((X0cs.T @ X1cs), full_matrices=False)
    R = (U @ V.T)
...
def se3_to_SE3(w: jnp.ndarray, u: jnp.ndarray) -> jnp.ndarray:
    wx = skew(w)
    theta = jnp.linalg.norm(w, axis=-1)[...,None,None]
    I = jnp.identity(3, dtype=w.dtype)
