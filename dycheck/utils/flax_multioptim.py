...
class MultiOptimizer(OptimizerDef):
  ...
  def update_hyper_params(self, **hyper_param_overrides):
    ...
    # extract lr
    hyper_param_overrides_list = [{'learning_rate': hyper_param_overrides['learning_rate'][0]}, {'learning_rate': hyper_param_overrides['learning_rate'][1]}]
    if hyper_param_overrides:
      hps = [hp.replace(**hyper_param_overrides_list[i]) for i, hp in enumerate(hps)]
    return hps
