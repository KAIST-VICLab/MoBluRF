...
class EmbedPosEnc(nn.Module):
    ...
    def __call__(
        self,
        xs: jnp.ndarray,
        metadata: Optional[jnp.ndarray] = None,
        alpha: Optional[float] = None,
        ...
        **_,
        ...
        if metadata is not None:
            ...
            return self.posenc(xs, alpha)

@gin.configurable(denylist=["name"])
class Pluecker(nn.Module):
    origin: float = gin.REQUIRED
    in_channels: int = 6
    out_channels: int = 6
    ...
    def __call__(
        ...
        # rays_o = rays_o - self.origin[None]
        rays_d = rays_d / jnp.linalg.norm(rays_d, axis=-1, keepdims=True)
        m = jnp.cross(rays_o, rays_d, axis=-1)
        return jnp.concatenate([rays_d * self.direction_multiplier, m * self.moment_multiplier], axis=-1)