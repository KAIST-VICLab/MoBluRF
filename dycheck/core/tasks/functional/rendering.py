...
def get_prender_image(
    ...
    **kwargs,
    ...
    def _model_fn(
        ...
        rays: struct.Rays,
        rngs: Dict[Literal["coarse_net", "static_net", "dynamic_net", "full"], types.PRNGKey],
...
def _render_image(
    _model_fn: Callable[
        [
            core.FrozenDict,
            struct.Rays,
            Dict[Literal["coarse_net", "static_net", "dynamic_net", "full"], types.PRNGKey],
        ],
        Dict[str, Dict[str, jnp.ndarray]],
    ],
    variables: core.FrozenDict,
    ...
    **_,
    ...
    rngs = jax.tree_map(
        lambda x: random.split(x, num_devices), {"static_net": key0, "dynamic_net": key1}
    )
    ...
    for i in (common.tqdm if show_pbar else lambda x, **_: x)(
        ...
        dynamic_out = out["dynamic_net"]
        static_out = out["static_net"]
        out = out["full"]
        out["dynamic_rgb"] = dynamic_out["rgb"]
        out["static_rgb"] = static_out["rgb"]
