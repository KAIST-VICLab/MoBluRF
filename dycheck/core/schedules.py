...
@gin.configurable()
class ConditionSchedule(Schedule):
    """Constant scheduler."""
    ...
    def get(self, step: int):
        # if step < self.value:
        if step % 2 == 0 and step <= self.value:
            return 0.0
        return 1.0

@gin.configurable()
class MinConstantSchedule(Schedule):
    """Constant scheduler."""

    def __init__(self, value: float, max_step:int):
        super().__init__()
...
@gin.configurable()
class ConditionLinearSchedule(Schedule):
    """Linearly scaled scheduler."""

    def __init__(
        self, initial_value: float, final_value: float, num_steps: int, start: int
    ):
        super().__init__()
    ...
    def get(self, step: int):
        ...
        clip_step = max(step - self.start, 0.0)
        alpha = min(clip_step / self.num_steps, 1.0)
