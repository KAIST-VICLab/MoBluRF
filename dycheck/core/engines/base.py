...
from dycheck.utils import common, struct, types, flax_multioptim, image
import numpy as np
...
class Engine(object):
    def __init__(
        ...
        **_,
        ...
        if self.stage == 'mdd':
            self.pretrained_dir = osp.join(self.work_root, self.dataset.dataset, self.dataset.sequence, "moblurf_bri/checkpoints")
    ...
    def build_model(self) -> Tuple[nn.Module, struct.TrainState]:
        ...
        rgbas = image.to_float32(
                np.array(
                    common.parallel_map(
                        self.dataset.parser.load_rgba, self.dataset.time_ids, self.dataset.camera_ids
                    )
                )
            )
        rgbs = rgbas[..., :3]
        ...
        model, variables = self.model_cls.create(
            ...
            scene_origin = self.dataset.center
        )
        
        if self.stage == 'mdd':
            logging.info(" Loading pretrained modes")
            self.pretrained_state = checkpoints.restore_checkpoint(self.pretrained_dir, None)

            variables = flax.core.frozen_dict.unfreeze(variables)
            variables['alpha'] = self.pretrained_state['optimizer']['target']['alpha']
            variables['params']['points_embed'] = self.pretrained_state['optimizer']['target']['params']['points_embed']
            variables['params']['nerfs_coarse_net'] = self.pretrained_state['optimizer']['target']['params']['nerfs_coarse_net']
            variables['params']['nerfs_static_net'] = self.pretrained_state['optimizer']['target']['params']['nerfs_static_net']
            variables['params']['nerfs_dynamic_net'] = self.pretrained_state['optimizer']['target']['params']['nerfs_dynamic_net']
            variables = flax.core.frozen_dict.freeze(variables)

            nerf_traversal = flax.optim.ModelParamTraversal(lambda path, _: 'ray_warping' not in path and 'pose_blurry' not in path)
            pose_traversal = flax.optim.ModelParamTraversal(lambda path, _: 'pose_blurry' in path or 'ray_warping' in path)

        elif self.stage == 'bri':
            nerf_traversal = flax.optim.ModelParamTraversal(lambda path, _: 'ray_warping' not in path)
            pose_traversal = flax.optim.ModelParamTraversal(lambda path, _: 'ray_warping' in path)
            
        else:
            raise NotImplementedError(f"Stage {self.stage} not supported.")

        optimizer_def = flax_multioptim.MultiOptimizer(
            (nerf_traversal, flax.optim.Adam(learning_rate=0.1)),
            (pose_traversal, flax.optim.Adam(learning_rate=0.01)),
        )
